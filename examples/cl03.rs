    ...
    <S::Ciphersuite as Ciphersuite>::HashAlg: Digest
{
    ...
    let commitment = Commitment::<CL03<S::Ciphersuite>>::commit_with_pk(&messages, issuer_keypair.public_key(), &a_bases, Some(&unrevealed_message_indexes));
    
    log::info!("Computation of a Zero-Knowledge proof-of-knowledge of committed messages");
    let zkpok = ZKPoK::<CL03<S::Ciphersuite>>::generate_proof(&messages, commitment.cl03Commitment(), None, issuer_keypair.public_key(), &a_bases, None, &unrevealed_message_indexes);

    log::info!("Verification of the Zero-Knowledge proof and computation of a blind signature");
    let blind_signature = BlindSignature::<CL03<S::Ciphersuite>>::blind_sign(issuer_keypair.public_key(), issuer_keypair.private_key(), &a_bases, &zkpok, Some(&revealed_messages), commitment.cl03Commitment(), None, None, &unrevealed_message_indexes, Some(&revealed_message_indexes));
    
    log::info!("Signature unblinding and verification...");
    let unblided_signature = blind_signature.unblind_sign(&commitment);
    let verify = unblided_signature.verify_multiattr(issuer_keypair.public_key(), &a_bases, &messages);
    ...
    let signature_pok = PoKSignature::<CL03<S::Ciphersuite>>::proof_gen(unblided_signature.cl03Signature(), &verifier_commitment_pk, issuer_keypair.public_key(), &a_bases, &messages, &unrevealed_message_indexes);
