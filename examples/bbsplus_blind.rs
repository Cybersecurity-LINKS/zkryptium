...
mod bbsplus_example {
    ...
    {
        const COMMITTED_MSGS: [&str; 2] = ["5982967821da3c5983496214df36aa5e58de6fa25314af4cf4c00400779f08c3", "a75d8b634891af92282cc81a675972d1929d3149863c1fc0"];
        ...
        let committed_messages: Vec<Vec<u8>> = COMMITTED_MSGS.iter().map(|m| hex::decode(m).unwrap()).collect();
        let (commitment_with_proof, secret_prover_blind) = Commitment::<BBSplus<S::Ciphersuite>>::commit(Some(&committed_messages))?;
        ...
        let blind_signature = BlindSignature::<BBSplus<S::Ciphersuite>>::blind_sign(issuer_sk, issuer_pk, Some(&commitment_with_proof.to_bytes()), Some(&header), Some(&messages), None)?;

        log::info!("Blind Signature Verification...");
        assert!(blind_signature.verify(issuer_pk, Some(&header), Some(&messages), Some(&committed_messages), Some(&secret_prover_blind), None).is_ok());
        ...
        let poks_verification_result = poks.blind_proof_verify(issuer_pk, Some(&all_disclosed_messages), Some(&all_disclosed_indexes), Some(&header), Some(&nonce_verifier)).is_ok();
        assert!(poks_verification_result, "Signature Proof of Knowledge Verification Failed!");
